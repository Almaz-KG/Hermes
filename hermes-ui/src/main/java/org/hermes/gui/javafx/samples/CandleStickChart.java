...
public class CandleStickChart extends XYChart<String, Number> {
    public static final String DEFAULT_DATE_FORMAT_PATTERN = "yyyy.MM.dd hh:mm";
    ...
    public CandleStickChart(QuoteHistory quoteHistory) {
        this(quoteHistory.getQuote().getQuoteName(), quoteHistory, Integer.MAX_VALUE);
    }

    public CandleStickChart(String title,  QuoteHistory quoteHistory, int maxBarsToDisplay) {
        this(title, new CategoryAxis(), new NumberAxis(), quoteHistory, maxBarsToDisplay);
    }

    public CandleStickChart(String title, CategoryAxis xAxis, NumberAxis yAxis,
                            QuoteHistory quoteHistory, int maxBarsToDisplay) {
        super(xAxis, yAxis);
        ...
        try {
            File file = IOUtils.getResourceFile(this.getClass(), "css/CandleStickChartStyles.css");
            getStylesheets().add(file.toURI().toURL().toExternalForm());
        ...
        List<Bar> subList = getSubList(quoteHistory, maxBarsToDisplay);

        for (int i = 0; i < subList.size(); i++) {
            Bar bar = subList.get(i);
            series.getData().add(new Data<>(sdf.format(bar.getDate()), bar.getOpen(), bar));
        }
    ...
    public void addBar(Bar bar) {
        ...
        String label = sdf.format(bar.getDate());


        lastBar = new Bar(bar.getDate(), bar.getClose(), bar.getClose(), bar.getClose(), bar.getClose(), BigDecimal.ZERO);
    ...
    protected List<Bar> getSubList(QuoteHistory history, int maxBars) {
    ...
    protected void updateAxisRange() {
        ...
        if (xData != null || yData != null) {
            for (Data<String, Number> data : dataSeries.get(0).getData()) {
                if (xData != null) {
